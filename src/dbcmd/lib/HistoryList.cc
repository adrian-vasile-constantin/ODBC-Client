...
namespace cmd
{
	...
	inline HistoryList(SIZE_TYPE capacity = DEFAULT_CAPACITY);
...
{
    auto it = std::ranges::find_if(std::views::iota(lines.begin(), lines.end()) | std::views::enumerate, [&line](auto enumerated)
	{
